  ...
  return (
    <div className="relative flex min-h-[calc(100vh-64px)] items-center justify-center bg-slate-50 px-4 py-12 dark:bg-slate-950 overflow-hidden">
      ...
      <div className="absolute inset-0 z-0 opacity-10">
         <div className="absolute bottom-0 right-0 h-96 w-96 bg-emerald-600 blur-[120px]" />
         <div className="absolute top-0 left-0 h-96 w-96 bg-emerald-600/30 blur-[120px]" />
      </div>
      ...
      >
        <div className="mb-8 text-center">
          ...
          <h2 className="mt-6 text-2xl font-bold text-slate-900 dark:text-white">Create your account</h2>
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
            Join the community of golfers playing for impact.
          </p>
        </div>
        ...
        <p className="mt-8 text-center text-xs text-slate-500 dark:text-slate-400">
          Already have an account? <Link href="/login" className="font-bold text-emerald-600 hover:text-emerald-500 underline">Sign in</Link>
        </p>
      </motion.div>
    </div>
  );
